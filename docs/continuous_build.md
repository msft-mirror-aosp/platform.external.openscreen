...
> [*] Tests run on Ubuntu 24.04, but are cross-compiled to arm64 with a debian
sysroot.

> [**] Currently non-functional.  See https://issuetracker.google.com/460509759

The `chromium_*` builders compile against Chromium top-of-tree to ensure that
changes can be autorolled into Chromium. Note that only a small fraction of bot
configurations are covered, so there is always a risk that the autoroll may fail
on a different bot configuration.
